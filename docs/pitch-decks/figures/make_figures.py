...
mpl.rcParams.update({
    ...
    "font.family": "serif",
    "font.size": 11,               # Increased for readability
...
def fig_signal_time():
    ...
    ax[1].set_title(r"\textbf{Ate\c{s} stresi}\quad$f_{\mathrm{dom}}\!\approx\!5$ Hz",
                    color=EMBER, fontsize=10, loc="left")
    for a in ax:
...
def fig_fft():
    ...
    ax.plot(fn, Xn, color=INDIGO, lw=1.2, label=r"Normal")
    ax.plot(ff, Xf, color=EMBER, lw=1.2, ls=(0, (5, 2)), label=r"Ate\c{s} stresi")
    ...
    ax.annotate(r"$20\,$Hz", xy=(fn[kn], Xn[kn]), xytext=(fn[kn]+2.5, Xn[kn]),
                color=INDIGO, fontsize=9, va="center",
                arrowprops=dict(arrowstyle="->", color=INDIGO, lw=0.9))
    ax.annotate(r"$5\,$Hz", xy=(ff[kf], Xf[kf]), xytext=(ff[kf]+2.5, Xf[kf]*0.98),
                color=EMBER, fontsize=9, va="center",
...
def fig_heat():
    ...
    gs = fig.add_gridspec(1, 2, width_ratios=[1.0, 1.0], wspace=0.3)
    ...
    cb = fig.colorbar(pm, ax=axm, pad=0.03)
    cb.set_label(r"$T\;[^{\circ}\mathrm{C}]$", fontsize=9)
    ...
    for zi, c, ls in [(0.05, INDIGO, "-"), (0.10, GREEN, "-"), (0.175, EMBER, "-")]:
        ...
        axc.plot(tmin, Tz, color=c, ls=ls, lw=1.4, label=fr"$z={int(zi*100)}$ cm")
    axc.set_xlabel(r"Zaman $t\;[\mathrm{dk}]$")
    axc.set_ylabel(r"$T\;[^{\circ}\mathrm{C}]$")
    axc.set_xlim(0, 90)
    axc.legend(loc="upper left", ncol=1, fontsize=8)
    axc.set_title(r"\textbf{Is\i\ ivmesi $dT/dt>\beta$}", loc="left", fontsize=10)
...
def fig_fusion():
    ...
    ax.scatter(nrm[:, 0], nrm[:, 1], s=12, color=GREEN, alpha=0.8, label=r"Normal", edgecolor="none")
    ax.scatter(fire[:, 0], fire[:, 1], s=16, color=EMBER, alpha=0.85, marker="^",
               label=r"Ate\c{s}", edgecolor="none")
    ax.axvline(bx, color=MUT, lw=1.0, ls=(0, (4, 3)))
    ax.axhline(by, color=MUT, lw=1.0, ls=(0, (4, 3)))
    xx = np.linspace(0, 6, 200)
    yy = 15 + 9*np.tanh((xx-2.0)*1.4)
    ax.plot(xx, yy, color=INDIGO, lw=1.6, label=r"LSTM s\i n\i r\i")
    ax.text(3.9, 36, r"\textbf{ALARM}", color=EMBER, fontsize=10, ha="center")
    ax.text(0.15, 37, r"$\beta_{T}$", color=MUT, fontsize=10)
    ...
    ax.set_title(r"\textbf{Sens\"or f\"uzyonu — ay\i rt edilebilirlik}", loc="left", fontsize=11)
...
def fig_roc():
    ...
    ax.plot([0, 1], [0, 1], color=MUT, lw=1.0, ls=":")
    ax.plot(f1, t1, color=INDIGO, lw=1.5, label=fr"Tek e\c{{s}}ik  (AUC$=${a1:.2f})")
    ax.plot(f2, t2, color=GREEN, lw=2.0, label=fr"Sens\"or f\"uzyonu  (AUC$=${a2:.2f})")
    i = np.argmin(np.abs(f2-0.01))
    ax.scatter([f2[i]], [t2[i]], s=36, color=EMBER, zorder=5)
    ax.annotate(r"\%1 yanl\i\c{s} alarm", xy=(f2[i], t2[i]), xytext=(0.28, 0.55),
                color=EMBER, fontsize=9,
                arrowprops=dict(arrowstyle="->", color=EMBER, lw=1.0))
    ax.set_xlim(0, 1); ax.set_ylim(0, 1.02)
    ax.set_xlabel(r"Yanl\i\c{s} alarm oran\i\ (FPR)")
    ax.set_ylabel(r"Do\u{g}ru tespit oran\i\ (TPR)")
    ax.legend(loc="lower right", fontsize=9)
    ax.set_title(r"\textbf{ROC — f\"uzyon vs. tek e\c{s}ik}", loc="left", fontsize=11)
    save(fig, "roc")
...
def fig_directional():
    ...
    ax.bar(ang, resp, width=np.deg2rad(30), color=INDIGO, alpha=0.35, edgecolor=INDIGO, lw=1.2)
    ax.plot([est, est], [0, 1.05], color=EMBER, lw=2.2)
    ax.plot([src, src], [0, 1.05], color=GREEN, lw=1.5, ls=(0, (4, 2)))
    ax.scatter(ang, resp, s=20, color=INDIGO, zorder=6)
    ax.set_rmax(1.15); ax.set_rticks([0.5, 1.0]); ax.set_yticklabels([])
    ax.set_xticks(np.deg2rad([0, 45, 90, 135, 180, 225, 270, 315]))
    ax.set_xticklabels([r"K", r"KD", r"D", r"GD", r"G", r"GB", r"B", r"KB"], fontsize=9)
...
def fig_breakeven():
    ...
    ax.plot(x, rev, color=GREEN, lw=2.0, label=r"Gelir $=360k\cdot n$")
    ax.plot(x, cost, color=EMBER, lw=2.0, label=r"Maliyet $=880k+160k\cdot n$")
    ax.axhline(880, color=MUT, lw=1.0, ls=(0, (4, 3)))
    ax.axvline(be, color=INDIGO, lw=1.2, ls=":")
    ax.scatter([be], [360*be], s=40, color=INDIGO, zorder=6)
    ax.annotate(r"Ba\c{s}a ba\c{s}: $n=4.4\!\to\!5$ paket", xy=(be, 360*be),
                xytext=(be+0.4, 360*be-620), fontsize=9,
    ...
    ax.set_title(r"\textbf{Ba\c{s}a ba\c{s} analizi}", loc="left", fontsize=11)
    save(fig, "breakeven")
...
def fig_electrode():
    ...
    for _ in range(24):
        ...
        ax.plot([x0, x1], [y0, y1], color=GREEN, lw=1.2, alpha=0.7)
        ax.plot(x1, y1, 'o', color=GREEN, ms=3.0, alpha=0.6)
    ...
    ax.annotate(layers[1][3], xy=(0.67, -0.67), xytext=(1.3, -1.45),
                fontsize=8.5, color=INDIGO, ha='center',
                arrowprops=dict(arrowstyle='->', color=INDIGO, lw=1.0))
    ...
    for i in range(8):
        ...
        ax.plot([0.52*np.cos(th), 1.38*np.cos(th)],
                ...
                color=INDIGO, lw=1.5, alpha=0.5, ls=(0,(5,3)))
        ax.plot(1.38*np.cos(th), 1.38*np.sin(th), 'o', color=INDIGO, ms=5, zorder=5)
    dirs = ['K','KD','D','GD','G','GB','B','KB']
    for i, d in enumerate(dirs):
        th = i * np.pi/4
        ax.text(1.62*np.cos(th), 1.62*np.sin(th), d,
                ha='center', va='center', fontsize=8, color=INK)
    ax.set_title(r"\textbf{\"U\c{c} katmanl\i\ biyomimetik k\"ok-elektrot}", fontsize=11.5, pad=14)
...
def fig_architecture():
    ...
    for x, y, w, h, c, title, sub in boxes:
        bb = FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.1",
                            facecolor=c, alpha=0.08, edgecolor=c, lw=1.8)
        ax.add_patch(bb)
    ...
    for i in range(3):
        ...
        ax.text((x0+x1)/2, ymid+0.2, labels[i], ha='center', fontsize=7.5, color=MUT)
    ax.text(4.15, 2.25, r"\textbf{Mycellium-Aegis --- Sistem Mimarisi}", ha='center', fontsize=11.5, color=INK)
...
def fig_ai_pipeline():
    ...
    for i, (num, title, eq, c) in enumerate(steps):
        ...
        bb = FancyBboxPatch((x, 0), w, h, boxstyle="round,pad=0.08",
                            facecolor=c, alpha=0.06, edgecolor=c, lw=1.8)
        ax.add_patch(bb)
        ax.text(x+w/2, h-0.2, num, ha='center', va='center', fontsize=8, color=c)
        ax.text(x+w/2, h*0.55, title, ha='center', va='center',
                fontsize=8.5, fontweight='bold', color=INK, linespacing=1.4)
        ax.text(x+w/2, 0.18, eq, ha='center', va='center', fontsize=7.5, color=MUT)
        if i < len(steps)-1:
    ...
    ax.text(3.9, -0.55, r"$\left(\frac{dT}{dt}>\beta\right) \;\wedge\; "
            ...
            ha='center', va='center', fontsize=9, color=EMBER,
            bbox=dict(boxstyle='round,pad=0.4', facecolor=EMBER, alpha=0.06, edgecolor=EMBER, lw=1.0))
    ax.set_title(r"\textbf{Sinyal i\c{s}leme hatt\i\\ --- ham gerilimden karara}", fontsize=11.5, pad=12)
